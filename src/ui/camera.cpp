...
glm::mat4 Camera::topDownViewMatrix() const { 
    glm::vec3 m_topDownPos = topDownPos();
    return glm::lookAt(m_topDownPos, m_topDownPos - m_up, m_forward);
}
...
void Camera::updateInput(MeshTree *mesh, MeshTree* root, glm::vec3 meshMiddleOffset) {
    if (m_userInteraction) {
        ...
        if (m_pWindow->isMouseButtonPressed(GLFW_MOUSE_BUTTON_LEFT)) {
            ...
            mesh->transform.selfRotate.w += glm::degrees(delta.x);
